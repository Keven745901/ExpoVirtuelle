...
window.onload = function () {
  ...
  let inputUploadOeuvre = document.querySelector("#uploadOeuvre");
  inputUploadOeuvre.onchange = function () {
    const url = "../scripts/upload.php";
    const form = new FormData();
    form.append("file", inputUploadOeuvre.files[0]);
  ...
  document.querySelector("#btnremplacer").onclick = function () {
    remplacerNumerosParModels(listeEmplacementsGeneres);
  };
  ...
  document
    ...
    .addEventListener("raycaster-intersected", function (e) {
      ...
      reader.onload = function (event) {
        if (window.DOMParser) {
          ...
          for (let m = 0; m < cylindres.length; m++) {
            ...
            console.log(cylindres[m].getAttribute("geometry"));
          }
